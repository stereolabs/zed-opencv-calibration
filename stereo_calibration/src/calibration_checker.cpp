...
CalibrationChecker::CalibrationChecker( cv::Size board_size, float square_size )
{
    ...
    board_.objp.clear();
    board_.board_size_mm = cv::Size(board_size.width * square_size,
                                    board_size.height * square_size);
...
bool CalibrationChecker::testSample(const std::vector<cv::Point2f>& corners, cv::Size image_size) {
    ...
    bool is_good = isGoodSample(params, corners, validCorners_.empty() ? std::vector<cv::Point2f>() : validCorners_.back());
    ...
    std::cout << std::setprecision(3) << "Sample: Pos(" << params.pos.x << ", " << params.pos.y << "), Size: " << params.size << ", Skew: " << params.skew << " => " << (is_good ? "Accepted" : "Rejected") << std::endl;

    return is_good; 
}

float CalibrationChecker::compute_skew(const std::vector<cv::Point2f>& outside_corners) {
    if (outside_corners.size() != 4) {
        return -1.0f; // Invalid input
    }
    
    auto angle = [](const cv::Point2f& a, const cv::Point2f& b, const cv::Point2f& c) -> float {
        cv::Point2f ab = a - b;
        cv::Point2f cb = c - b;
    ...
    float skew =  std::min(1.0f, 2.0f * std::abs(PI/2 - angle(outside_corners[up_left], outside_corners[up_right], outside_corners[down_right])));

    return skew;
}

float CalibrationChecker::compute_area(const std::vector<cv::Point2f>& outside_corners) {
    if (outside_corners.size() != 4) {
        return -1.0f; // Invalid input
    }
...
std::vector<cv::Point2f> CalibrationChecker::get_outside_corners(
    ...
    outside_corners[down_right] = corners[(y_dim - 1) * x_dim + (x_dim - 1)];   // Bottom-right
    outside_corners[down_left] = corners[(y_dim - 1) * x_dim];                  // Bottom-left
    
    return outside_corners;
}

BoardParams CalibrationChecker::getDetectedBoarParams(const std::vector<cv::Point2f>& corners, cv::Size image_size) {
    BoardParams params;
...
void CalibrationChecker::calculateSampleCollectionStatus(float& size_score, float& skew_score,
                                     float& pos_score_x,
