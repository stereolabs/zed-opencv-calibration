...
typedef struct _board {
    cv::Size board_size = {0, 0};               // Number of inner corners per a chessboard row and column
    float square_size = 0.0f;                   // Size of a square in your defined unit (point, millimeter,etc).
...
typedef struct _board_params {
    cv::Point2f pos = {-1.0f, -1.0f};   // Normalized position of the checkerboard in the image
...
public:
   ...
   private:
    ...
    // Helper functions
    std::vector<cv::Point2f> get_outside_corners(const std::vector<cv::Point2f>& corners); // Get the 4 outside corners of a checkerboard
    float compute_skew(const std::vector<cv::Point2f>& outside_corners); // Compute skew based on the 4 outside corners
    float compute_area(const std::vector<cv::Point2f>& outside_corners); // Compute area based on the 4 outside corners
